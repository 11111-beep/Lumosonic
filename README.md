...
- **在线音乐搜索**: 集成WYY音乐 API，海量曲库即搜即播，并自动加载高清专辑封面。
...
1.从功能上看，LumoSonic在实现第三方平台音乐播放的基础上也支持对本地所有音乐文件的扫描。
现在市面上普遍存在两种音乐APP
- 官方的音乐平台
- 本地音乐播放器

LumoSonic将其两者结合,使不同平台和本地歌曲自由播放的问题得到解决。


2.从架构上看LumoSonic 的核心是其 **双进程架构**，通过 `AIDL` 实现跨进程通信（IPC）。
...
| 分类           | 技术                        | 描述                                                   |
| -------------- | --------------------------- | ------------------------------------------------------ |
| **语言**       | Kotlin & Java               | 主要采用 Java 编写，充分利用其现代化语言特性。       |
